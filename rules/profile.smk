...
if config["params"]["metaphlan3"]["do"] and config["params"]["kraken2"]["do"]:
    rule all:
        input:
    ...
    rule all:
        input:
            expand("{result_dir}/filter_summary.txt", result_dir = config["results"]),
            expand("{result_dir}/bracken.merged.abundance.profile.{level}.tsv", result_dir = config["results"], level=config["params"]["bracken"]["level"])
elif config["params"]["metaphlan3"]["do"]:
    rule all:
        input:
            expand("{result_dir}/filter_summary.txt", result_dir = config["results"]),
            expand("{result_dir}/metaphlan3.profile.merge.txt", result_dir = config["results"])
...
rule metaphlan3:
    ...
    output:
        mpa = protected(os.path.join(config["assay"]["profile"]["metaphlan3"], "{sample}.mp3.profile")),
        vir = protected(os.path.join(config["assay"]["profile"]["metaphlan3"], "{sample}.mp3_vir.profile"))
    threads:
        config["params"]["metaphlan3"]["threads"]
    params:
        bowtie2db = config["params"]["metaphlan3"]["bowtie2db"],
        index = config["params"]["metaphlan3"]["index"],
        bw2 = protected(os.path.join(config["assay"]["profile"]["metaphlan3"], "{sample}.mp3.bw2.bz2"))
    log:
        os.path.join(config["logs"]["profile"], "{sample}.metaphlan3.log")
    benchmark:
        os.path.join(config["benchmarks"]["profile"], "{sample}.mp3.benchmark.txt")
    shell:
        '''
        metaphlan {input.rmhost_r1},{input.rmhost_r2} --bowtie2db {params.bowtie2db} --index {params.index} --nproc {threads} --input_type fastq --bowtie2out {params.bw2} -o {output.mpa} 2> {log}
        #-t rel_ab_w_read_stats > {output.mpa}
        metaphlan {params.bw2} --bowtie2db {params.bowtie2db} --index {params.index} --nproc {threads} --input_type bowtie2out --add_viruses > {output.vir}
        #-t rel_ab_w_read_stats > {output.vir}
        '''


rule merge_profile_mp3:
    input:
        mpa = expand("{profile_dir}/{sample}.mp3.profile", profile_dir = config["assay"]["profile"]["metaphlan3"], sample = _samples.index),
        vir = expand("{profile_dir}/{sample}.mp3_vir.profile", profile_dir = config["assay"]["profile"]["metaphlan3"], sample = _samples.index)
    output:
